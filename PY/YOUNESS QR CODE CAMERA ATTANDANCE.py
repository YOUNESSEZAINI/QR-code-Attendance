...
class AttendanceControlApp:
    def __init__(self, root):
        ...
        self.btn_generate_pdf = ttk.Button(root, text="Générer un rapport PDF", command=self.generate_pdf_report)
        self.btn_generate_xlsx = ttk.Button(root, text="Générer un rapport XLSX", command=self.generate_xlsx_report)
    ...
    def on_treeview_click(self, event):
        ...
        confirmation = messagebox.askyesno("Supprimer l'enregistrement", f"Do you want to delete the record with QR Data: {qr_data}?")
        if confirmation:
            self.delete_record()
